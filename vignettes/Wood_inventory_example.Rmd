...
N_HHS2020 <- bind_rows(
  ...
  dplyr::select(site, town, N_HHS2020)
```
...
PERC_HHS_WOOD_SUMMR_CHS2020 <- bind_rows(
  ...
  mutate(PERC_HHS_WOOD_SUMMR_CHS2020 = map2(n, PointEst, ~maak_distlist(
    ...
    site = Hmisc::capitalize(site),
    town = Hmisc::capitalize(town)) %>%
  dplyr::select(site, town, PERC_HHS_WOOD_SUMMR_CHS2020) %>% 
...
PERC_HHS_WOOD_SUMMR_CHS2020 %>% 
  ...
  dplyr::select(-PERC) %>% kable(digits = 5)
```
...
```{r woodusers, fig.cap="\\label{fig:woodusers} Estimated number of whood users per town", fig.width=16, out.width="100%"}
...
```{r}
dfWoodUsers %>% dplyr::select(site, town, res) %>% unnest(res)
```
...
specs <- map(dfWoodUsers$res, 
  ~maak_distlist(
    nm = "N_Whh",
    dist = "unif",
    params = fitdist(.$Result, "unif")$estimate %>% round())
  )
```
...
xh <- bind_rows(
  ...
  dplyr::select(site, town, response, nn, n, perc) %>%
...
The mass of wood consumed by a wood using household per summer month is modelled by a zero-inflated poison distribution. The parameters were given in Table \ref{fig:kgwoodchssummer}. 
...
dfSummerCHS %>% dplyr::select(site, town, ss) %>% unnest(ss) %>% group_by(site, town) %>% kable(digit = 0)
```
...
```{r fpdkab, fig.cap="\\label{Number of fires per day}"}
...
N_FIRES_PWOODHH_PDAY_SUMMR_FLB2020 %<>%
  mutate(N_fpd = purrr::map2(Median, `Std. Dev`, ~maak_distlist(
    nm = "N_fpd",
    dist = "pois",
    params = c(lambda = ..1))
    )
    ) %>%
  dplyr::select(site, town, N_fpd)
...
## Fires per day x Kg per fire

```{r}
dfFireMonth <- N_FIRES_PWOODHH_PDAY_SUMMR_FLB2020  %>% 
  left_join(KG_WOOD_PWOODHH_PFIRE_SUMMR_M1_WWS2020) %>% 
  filter(town == "Sefikile_p2" | town == "Mfidikoe_sp") %>% 
  mutate(ll  = pmap(., ~list( N_fpd=..3,  KG = ..4)),
         res = map2(ll, town, ~monticarleer(..1, " N_fpd * 30.4 * KG_WOOD_PWOODHH_PFIRE_SUMMR_M1_WWS2020",
                                            tbl_out = FALSE,
                                            saveplot = saveplotvar,
                                            fn = paste0("../man/figures/dfFireMonth_WWS_", ..2, ".pdf")) )
  ) %>% unnest(res)%>%
  mutate(ss = map(res, ~broom::tidy(summary(.$Result))))
```
...
```{r out.width="100%", fig.width=16}
dfSummerWWS <- dfWoodUsers  %>% 
  left_join(KG_WOOD_PWOODHH_PFIRE_SUMMR_M1_WWS2020) %>% 
  left_join(N_FIRES_PWOODHH_PDAY_SUMMR_FLB2020) %>% 
  filter(town == "Sefikile_p2" | town == "Mfidikoe_sp") %>% 
  mutate(ll  = pmap(., ~list( N_Whh=..3,  KG = ..4, N_fpd = ..5 )),
         res = map2(ll, town, ~monticarleer(..1, "N_Whh * (KG_WOOD_PWOODHH_PFIRE_SUMMR_M1_WWS2020 * N_fpd * 30.4)",
                                            tbl_out = FALSE,
                                            saveplot = saveplotvar,
...
```{r}
dfSummerWWS %>% dplyr::select(site, town, ss) %>% unnest(ss) %>% group_by(site, town) %>% kable(digit = 0)
```
...
## Monthly mass of wood in winter
...
KG_WOOD_PWOODHH_PFIRE_SUMMR_M2_WWS2020 <- bind_rows(
  ...
  mutate(KG_WOOD_PWOODHH_PFIRE_SUMMR_M2_WWS2020 = map(Mean, ~maak_distlist(
    ...
    dist = "pois" ,
    params = c(lambda = .) ))) %>%
  dplyr::select(site, town, KG_WOOD_PWOODHH_PFIRE_SUMMR_M2_WWS2020)
```
...
N_FIRES_PWOODHH_PMONTH_SUMMR_FLB2020 <- bind_rows(
  ...
  mutate(N_FIRES_PWOODHH_PMONTH_SUMMR_FLB2020 = map(Mean, ~maak_distlist(
    ...
    dist = "pois" ,
    params = c(lambda = .) ))) %>%
  dplyr::select(site, town, N_FIRES_PWOODHH_PMONTH_SUMMR_FLB2020)
```
...
N_FIRES_PWOODHH_PMONTH_SUMMR_FLB2020 %>% 
  ...
  dplyr::select(-N_FIRES_PWOODHH_PMONTH_SUMMR_FLB2020) %>% 
  kable()
```
